...
export default function ManagerDashboard() {
  ...
  return (
    <div style={{ display: 'grid', gap: 'var(--sp-5)' }}>
      ...
      {/* ── KPI + Charts (Bento Grid) ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 'var(--sp-5)' }}>
        <Card $hoverable style={{ gridColumn: 'span 5', padding: 'var(--sp-5)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <p className="meta" style={{ fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Team Avg Progress</p>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: 'var(--sp-4)' }}>
            <div>
              <div style={{ fontSize: '2.5rem', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 1, letterSpacing: '-0.03em' }}>{avgProgress}%</div>
              <div className="meta" style={{ color: 'var(--text-muted)', marginTop: 6, fontWeight: 500 }}>Target: 75% by end-of-month</div>
            </div>
            <div style={{ width: 100 }}>
              <Sparkline data={SPARK} dataKey="v" color="var(--chart-blue)" height={48} showDot={false} />
            </div>
          </div>
        </Card>
        <Card $hoverable style={{ gridColumn: 'span 7', padding: 'var(--sp-5)' }}>
          <h2 className="h3" style={{ marginBottom: 'var(--sp-4)' }}>Team by Phase</h2>
          <StackedBarChart
            data={PHASE_DATA}
            xKey="phase"
        ...
        <AnimatedList style={{ display: 'grid', gap: 8 }}>
          {visible.map(m => (
            <AnimatedItem key={m.name}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', borderRadius: 'var(--r-md)', border: '1px solid var(--border-subtle)' }}>
                <Avatar name={m.name} size={40} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: '0.9375rem', color: 'var(--text-primary)' }}>{m.name}</div>
                  <div className="meta" style={{ color: 'var(--text-muted)', marginTop: 2, fontWeight: 500 }}>{m.role}</div>
                </div>
                <div style={{ width: 120 }}>
                  ...
                  <div className="meta" style={{ marginTop: 4, textAlign: 'right', color: 'var(--text-muted)', fontWeight: 500 }}>{m.progress}%</div>
                </div>
